...
def get_client_model(input_dim: int) -> tf.keras.Model:
    ...
    # LayerNormalization keeps per-sample statistics (no running mean/variance), so the
    # client-side weights remain valid after cross-hospital averaging in SplitFed.
    ...
    x = tf.keras.layers.LayerNormalization(name="client_ln_64")(x)
    ...
    outputs = tf.keras.layers.LayerNormalization(name="client_ln_32")(outputs)
